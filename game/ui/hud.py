...
_NOISE_LABELS: dict[tuple[int, int, int] | None, str] = {
    ...
    COLOUR_NOISE_SLOW:  "Silent",
    COLOUR_NOISE_FAST:  "Loud",
}
...
class HUD:
    """
    Draws:
    - Objective checklist (top-right) with bounce animation on completion
    - Noise level colour dot
    - Timing bar (bottom-centre, via ObjectiveManager)
    - "Return to Gramps!" prompt once all objectives done
    - Intel mini-map showing all active dealers
    """
    ...
    def draw(
        ...
        dealer_positions: list[tuple[int, int]],
    ) -> None:
        ...
        if obj_manager.intel_active:
            self._draw_intel_minimap(surface, tractor_pos, dealer_positions)
    ...
    def _draw_checklist(
        ...
        n_rows  = 3
        panel_h = n_rows * _ROW_H + _ROW_H + 12
        ...
        for obj_type in obj_manager.order:
            ...
            # Bounce offset when just completed
            flash = obj_manager.completion_flash(obj_type)
            bounce_y = int(-6 * math.sin(math.pi * flash / 0.6)) if flash > 0 else 0

            text   = self._font_sm.render(f"{tick} {label}", True, colour)
            shadow = self._font_sm.render(f"{tick} {label}", True, (0, 0, 0))
            surface.blit(shadow, (x + 1, y + bounce_y + 1))
            surface.blit(text,   (x,     y + bounce_y))
            y += _ROW_H
    ...
    def _draw_intel_minimap(
        ...
        for pos in dealer_positions:
            pygame.draw.circle(mm, MINIMAP_DEALER_COLOUR,
                (int(pos[0] * s), int(pos[1] * s)), 5)
